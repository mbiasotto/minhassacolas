<!-- Help Center Section -->
<section class="py-5">
    <div class="container text-center">
        <h2 class="display-5 mb-2 animate animate-up">Central de Ajuda</h2>
        <p class="text-pink mb-5 animate animate-up">Encontre aqui todo o suporte necessário para seus projetos</p>
        
        <div class="row g-4">
            <div class="col-md-6 animate animate-up">
                <div class="mb-3">
                    <i class="bi bi-book display-4 text-pink"></i>
                </div>
                <h3 class="h4 mb-3">Como Comprar</h3>
                <p class="mb-3">Aprenda passo a passo como fazer seus pedidos corretamente</p>
                <a href="./comprar.php" class="btn btn-clean">Ver mais</a>
            </div>
            
            <!--
            <div class="col-md-4">
                <div class="mb-3">
                    <i class="bi bi-rulers display-4 text-pink"></i>
                </div>
                <h3 class="h4 mb-3">Gabaritos para Download</h3>
                <p class="mb-3">Modelos prontos para seus arquivos em diversos formatos</p>
            </div>
-->
            
            <div class="col-md-6 animate animate-up">
                <div class="mb-3">
                    <i class="bi bi-gear display-4 text-pink"></i>
                </div>
                <h3 class="h4 mb-3">Suporte Técnico</h3>
                <p class="mb-3">Orientações técnicas para preparação de arquivos</p>
                <a href="https://wa.me/5515974020934?text=Olá,%20preciso%20de%20ajuda%20com%20os%20gabaritos." target="_wp" rel="nofollow" class="btn btn-clean">Obter ajuda</a>
            </div>
        </div>

        <!--<div class="mt-5 pt-4">
            <p class="mb-3">Precisa de ajuda adicional? Entre em contato com nossa equipe de suporte</p>
            <button class="btn btn-outline-dark px-4">Contatar Suporte</button>
        </div>-->
    </div>
</section>
